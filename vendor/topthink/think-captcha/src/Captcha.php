...
class Captcha
{
    ...
    //随机字体
    public function random_ttf($ttfPath){
        $dir  = dir($ttfPath);
        $ttfs = [];
    ...
    public function create(string $config = null, bool $api = false): Response
    {
        ...
        // 图片宽(px)
        $this->imageW || $this->imageW = $this->length * $this->fontSize * 1 + $this->length * $this->fontSize / 3;
        // 图片高(px)
        ...
        // 验证码字体随机颜色
        $this->color = imagecolorallocate($this->im, mt_rand(50, 150), mt_rand(100, 150), mt_rand(150, 200));

        // 验证码使用随机字体
        $ttfPath = __DIR__ . '/../assets/' . ($this->useZh ? 'zhttfs' : 'ttfs') . '/';

        $this->fontttf = $this->random_ttf($ttfPath);
    ...
    private function createLine()
    {
        //线条
        for ($i = 0; $i < 10; ++$i) {
            $color = imagecolorallocate($this->im, mt_rand(200, 210), mt_rand(190, 200), mt_rand(180, 190));
            imageline($this->im, mt_rand(0, $this->imageW), mt_rand(0, $this->imageH), mt_rand(0, $this->imageW), mt_rand(0, $this->imageH), $color);
        }
        //雪花
        for ($i = 0; $i < 50; ++$i) {
            $color = imagecolorallocate($this->im, mt_rand(190, 200), mt_rand(180, 190), mt_rand(200, 210));
            imagestring($this->im, mt_rand(0, 50), mt_rand(0, $this->imageW), mt_rand(0, $this->imageH), '-', $color);
        }
    }
    ...
    protected function writeNoise(): void
    {
        ...
        for ($i = 0; $i < 2; $i++) {
            //杂点颜色
            $noiseColor = imagecolorallocate($this->im,mt_rand(200, 210), mt_rand(190, 200), mt_rand(180, 190));
            // 验证码使用随机字体
            $ttfPath = __DIR__ . '/../assets/' . ($this->useZh ? 'zhttfs' : 'ttfs') . '/';
            for ($j = 0; $j < 5; $j++) {
                // 绘杂点
                $fontttf = $ttfPath.$this->random_ttf($ttfPath);
                $angle = $this->math ? 0 : mt_rand(-40, 40);
                imagettftext($this->im, 16, $angle, mt_rand(-10, (int)$this->imageW), mt_rand(-10, (int)$this->imageH), $noiseColor, $fontttf, $codeSet[mt_rand(0, 29)]);
            }
        }
    }
