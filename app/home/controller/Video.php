...
class Video extends IndexBase
{
    ...
    protected function initialize()
    {
        ...
        if($action == "detail"){
            ...
            View::assign("article", $video);
            $this->columnModel = Column::find($video['column_id']);//栏目模型
        }else{
    ...
    {
        $tempHtml = "list_video.{$this->view_suffix}";
        if($this->columnModel){
    ...
    public function  detail(){
        $model_template = "view_video.{$this->view_suffix}";
        if($this->columnModel){
