...
class Download extends IndexBase
{
    ...
    protected function initialize()
    {
        ...
        if($action == "detail"){
            ...
            View::assign("article", $download);
            $this->columnModel = Column::find($download['column_id']);//栏目模型
        }else{
    ...
    {
        $tempHtml = "list_download.{$this->view_suffix}";
        if($this->columnModel){
    ...
    public function  detail(){
        $model_template = "view_download.{$this->view_suffix}";
        if($this->columnModel){
