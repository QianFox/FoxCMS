...
class Link extends AdminBase
{
    public function index($page=1, $pageSize=100)
    {
        ...
        if($this->request->isAjax()){
            ...
            $list = (new LinkModel())->where($where)->order(['sort'=>'desc', 'create_time'=>'desc'])->paginate(['page'=> $page, 'list_rows'=>$pageSize]);
    ...
    public function add()
    {
        ...
        array_push($breadcrumb, ['id'=>'', 'title'=>'添加幻灯片', 'name'=>DIRECTORY_SEPARATOR. config('adminconfig.admin_path').'/Link/add','url'=>'javascript:void(0)']);
        View::assign("breadcrumb", $breadcrumb);

        if ($this->request->isPost()) {
            $param = $this->request->param();
            $param['lang'] = $this->getMyLang();
            LinkModel::create($param);
            $this->success("操作成功");
        }
        return view('add');
    }
    ...
    public function edit()
    {
        ...
        $bcidStr = str_replace(",","_", $authRule->tier);
        $breadcrumb = AuthRule::getBreadcrumb($bcidStr);
        array_push($breadcrumb, ['id'=>'', 'title'=>'编辑幻灯片', 'name'=>DIRECTORY_SEPARATOR. config('adminconfig.admin_path').'/Link/edit','url'=>'javascript:void(0)']);
        View::assign("breadcrumb", $breadcrumb);
