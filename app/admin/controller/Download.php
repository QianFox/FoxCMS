...
class Download extends AdminContentBase
{
    public function initialize()
    {
        ...
        if ($action == "add" || $action == "edit") {
            ...
            //服务器名称
            $serverNameList = DictTypeModel::where('dict_type', 'server_name')->with('dictDatas')->find()->dictDatas; //服务器名称
            $serverNames = [];
            $serverNameCode = 0;
            if (sizeof($serverNameList) == 1) {
                $serverName = $serverNameList[0];
                $dict_value = $serverName['dict_value'];
                $serverNames = explode(',', $dict_value);
                $serverNameCode = $serverName['dict_code'];
            }
            View::assign('serverNames', $serverNames);
            View::assign('serverNameCode', $serverNameCode);
            //文件类别
            $fileCategoryList = DictTypeModel::where('dict_type', 'file_category')->with('dictDatas')->find()->dictDatas; //文件类别
            $fileCategorys = [];
            $fileCategoryeCode = 0;
            if (sizeof($fileCategoryList) == 1) {
                $fileCategory = $fileCategoryList[0];
                $dict_value = $fileCategory['dict_value'];
                $fileCategorys = explode(',', $dict_value);
                $fileCategoryeCode = $fileCategory['dict_code'];
            }
            View::assign('fileCategorys', $fileCategorys);
            View::assign('fileCategoryeCode', $fileCategoryeCode);

            //查询栏目
            $columnDatas  = Column::where(["column_model" => "article"])->field('id,name as title')->order('level asc')->order('sort asc')->select(); //栏目
            ...
            if (sizeof($attachments) == 1) {
                if (!empty($attachments[0]['f_suffixs'])) {
                    $f_suffixs = $attachments[0]['f_suffixs'];
                    $f_suffixsArr = explode(",", $f_suffixs);
                    $fs = "";
                    foreach ($f_suffixsArr as $fsuffix) {
                        $fs .= "." . $fsuffix . ",";
                    }
                    $f_suffixs = substr($fs, 0, strlen($fs) - 1);
                }
            }
            View::assign('f_suffixs', $f_suffixs);
        }
    }
    ...
    public function index()
    {
        ...
        if ($this->request->isAjax(true)) {
            $where = array();
            $download = new \app\common\model\Download();
            ...
            //向下查询所有子孙栏目
            $columnIdArr = getChildrensColumnId($columnId, 'Download', $this->getMyLang());
            ...
            $list = $download->where($where)->order("create_time", "desc")->paginate(['page' => $param['currentPage'], 'list_rows' => $param['pageSize']]);
    ...
    public function add()
    {
        ...
        array_push($breadcrumb, ['id' => '', 'title' => '添加', 'name' => DIRECTORY_SEPARATOR . config('adminconfig.admin_path') . '/Download/add', 'url' => 'javascript:void(0)']);
        ...
        if ($this->request->isAjax()) {
            ...
            if (!empty($param['file_id'])) {
                $uf = \app\common\model\UploadFiles::find($param['file_id']);
                if ($uf) {
                    $param['file_name'] = $uf['file_name'];
                    $param['file_size'] = $uf['file_size'];
                    $uf->delete();
                }
            }
            unset($param['file_id']);
            $param['lang'] = $this->lang;
            $download = \app\common\model\Download::create($param);
            xn_add_admin_log("添加下载", "download", $param['title']); //添加日志
            $seo = xn_cfg("seo");
            if ($seo["url_model"] == 3) {
                $rparam =  ["columnId" => $param["column_id"], "oneId" => "key3", "first" => 1, "column_model" => "download", "id" => $download->id];
    ...
    public function edit()
    {
        ...
        $bcidStr = '4_' . str_replace(",", "_", $column->tier);
        $breadcrumb = Column::getBreadcrumb($bcidStr);
        array_push($breadcrumb, ['id' => '', 'title' => '编辑', 'name' => DIRECTORY_SEPARATOR . config('adminconfig.admin_path') . '/Download/edit', 'url' => 'javascript:void(0)']);
        View::assign("breadcrumb", $breadcrumb);

        View::assign('id', $param['id']);
        if (array_key_exists('id', $param)) {
            $download = \app\common\model\Download::find($param['id']);
            $articleFieldList = $this->getAttrListAttr($download->article_field);
        ...
        if ($this->request->isAjax()) {
            ...
            $fc = \app\common\model\Download::field("id,create_time")->find($param['id']);
            if ($fc) {
            ...
            $param['lang'] = $this->lang;
            \app\common\model\Download::update($param);
            xn_add_admin_log("编辑下载", "download", $param['title']); //添加日志
            $seo = xn_cfg("seo");
            if ($seo["url_model"] == 3) {
                $rparam =  ["columnId" => $param["column_id"], "oneId" => "key3", "first" => 1, "column_model" => "download", "id" => $param["id"]];
                $rparam = array_merge($rparam, $seo);
                $this->success('操作成功', "", $rparam);
            } else {
                $this->success('操作成功');
    ...
    public function batchAddField()
    {
        ...
        if (array_key_exists('articleField', $param) && array_key_exists('ids', $param)) {
            ...
            try {
                $res = \app\common\model\Download::whereIn("id", $idsArray)->update(['article_field' => $param['articleField']]);
                if ($res) {
                    $this->success("操作成功");
                } else {
    ...
    public function batchDelField()
    {
        ...
        if (array_key_exists('ids', $param)) {
            ...
            try {
                $res = \app\common\model\Download::whereIn("id", $idsArray)->update(['article_field' => '']);
                if ($res) {
                    $this->success("操作成功");
                } else {
                    $this->error("未找到要更新的数据");
    ...
    public function batchMove()
    {
        ...
        if (array_key_exists('columnId', $param) && array_key_exists('ids', $param)) {
            ...
            try {
                $res = \app\common\model\Download::whereIn("id", $idsArray)->update(['column_id' => $param['columnId']]);
                if ($res) {
                    xn_add_admin_log("批量移除下载", "download"); //添加日志
                    $this->success("操作成功");
                } else {
...
public function batchCope()
{
    ...
    if (array_key_exists('ids', $param)) {
        ...
        try {
            // 使用ORM方法查询数据
            $downloads = \app\common\model\Download::whereIn('id', $idsArray)->select();
            ...
            // 批量插入
            if (!empty($insertData)) {
                (new \app\common\model\Download())->saveAll($insertData);
                xn_add_admin_log("批量复制下载", "download");
                $this->success("操作成功");
            } else {
    ...
    public function batchDel()
    {
        ...
        if (array_key_exists('ids', $param)) {
            ...
            try {
                $res = \app\common\model\Download::whereIn('id', $idsArray)->delete();
                if ($res) {
                    xn_add_admin_log("批量删除下载", "download"); //添加日志
                    $this->success("操作成功");
                } else {
    ...
    {
        $download = \app\common\model\Download::find($id);
        $model = "";
        $columnId = $download->column_id;
    ...
    public function clickField()
    {
        ...
        $article = \app\common\model\Download::find($param['id']);
        if (!$article) {
            $this->error("没找到文章数据");
        }
        ...
        $r = \app\common\model\Download::update(['id' => $param['id'], 'article_field' => implode(",", $article_fields)]);
        if ($r <= 0) {
            $this->error("操作失败");
        }
        $this->success("操作成功");
    ...
    public function import()
    {
        ...
        if (!$fileUtil->validation($file)) {
            $this->error("请检查上传文件的后缀格式或大小是否符合设置！");
        }
        //文件上传
        $result = $fileUtil->upload($file, 0);
        if ($result['code']  == 1) {
            $file = $result['file'];
            ...
            $result['file_size'] = xn_file_size($result['file_size']);
