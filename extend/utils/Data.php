...
final class Data
{
    ...
    static public function channelLevel($data, $pid = 0, $html = "&nbsp;", $fieldPri = 'cid', $fieldPid = 'pid', $level = 1)
    {
        ...
        foreach ($data as $v) {
            if(!empty($v["name"])){
//                $v["name"] = DIRECTORY_SEPARATOR.config("adminconfig.admin_path").$v["name"];
                $v["name"] = url(config("adminconfig.admin_path").$v["name"]);
            }
            if ($v[$fieldPid] == $pid) {
                $arr[$v[$fieldPri]] = $v;
     ...
     * @return array
     */
    static public function channelLevelChild($data, $pid = 0, $html = "&nbsp;", $fieldPri = 'cid', $fieldPid = 'pid', $level = 1)
    {
        if (empty($data)) {
            return array();
        }
        ...
        foreach ($data as $v) {
            if ($v[$fieldPid] == $pid) {
                ...
                $arr[$v[$fieldPri]]['_level'] = $level;
                $arr[$v[$fieldPri]]['_html'] = str_repeat($html, $level - 1);
                $arr[$v[$fieldPri]]["_data"] = self::channelLevelChild($data, $v[$fieldPri], $html, $fieldPri, $fieldPid, $level + 1);
            }
        }
        return $arr;
    }
